              ...
              children: [
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(_greeting(), style: const TextStyle(fontFamily: 'Mukta', fontSize: 12, color: MitraColors.textMuted)),
                        Text('$firstName 👋', style: const TextStyle(fontFamily: 'Baloo2', fontWeight: FontWeight.w800, fontSize: 20, color: MitraColors.textPrimary)),
                      ],
                    ),
                    Container(
                      width: 44, height: 44,
                ...
                Container(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(fontFamily: 'Mukta', fontWeight: FontWeight.w500, fontSize: 11, color: MitraColors.saffron),
                  ),
                ),
                const SizedBox(height: 8),
                // Stats row
              ...
              child: Column(
                children: [
                  ...
                  _Section(
                    ...
                    trailing: const Text('See all', style: TextStyle(fontSize: 12, color: MitraColors.saffron, fontFamily: 'Mukta')),
                  ...
                  _Section(
                    ...
                    child: Row(
                      children: [
                        _QuickAction(emoji: '🥽', label: 'Open AR',     onTap: () => context.go('/student/ar')),
                        _QuickAction(emoji: '📝', label: 'Take Quiz',   onTap: () => context.go('/student/learn')),
                        _QuickAction(emoji: '🏆', label: 'Leaderboard', onTap: () => context.go('/student/ranks')),
                        _QuickAction(emoji: '📴', label: 'Download',    onTap: () => context.go('/student/learn')),
                      ],
                    ),
                  ),
                  ...
                  _Section(
                    ...
                    trailing: GestureDetector(
                      ...
                      child: const Text('View all', style: TextStyle(fontSize: 12, color: MitraColors.saffron, fontFamily: 'Mukta')),
                    ),
                    child: Column(
                      children: [
                        _RankRow(rank: '🥇', name: firstName, xp: user?.totalXp ?? 1200, isMe: true),
                        const SizedBox(height: 8),
                        const _RankRow(rank: '🥈', name: 'Rahul S.', xp: 980, isMe: false),
                        const SizedBox(height: 8),
                        const _RankRow(rank: '🥉', name: 'Priya M.', xp: 860, isMe: false),
                      ],
                    ),
                  ),
                ],
...
class _StatChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Text(text, style: const TextStyle(fontFamily: 'SpaceMono', fontSize: 11, color: MitraColors.textSecondary)),
  );
}
    ...
    child: Column(
      children: [
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
            Text(title, style: const TextStyle(fontFamily: 'Baloo2', fontWeight: FontWeight.w600, fontSize: 16, color: MitraColors.textPrimary)),
    ...
    child: Column(
      ...
      children: [
        Container(height: 3, decoration: const BoxDecoration(
          ...
          borderRadius: BorderRadius.only(topLeft: Radius.circular(MitraRadius.md), topRight: Radius.circular(MitraRadius.md)),
        )),
        Padding(
          padding: const EdgeInsets.all(MitraSpacing.md),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const Text('SCIENCE · CHAPTER 3', style: TextStyle(fontFamily: 'Mukta', fontSize: 10, color: MitraColors.textMuted, letterSpacing: 1)),
              const SizedBox(height: 4),
              const Text('Microscopy & Cell Structure', style: TextStyle(fontFamily: 'Baloo2', fontWeight: FontWeight.w600, fontSize: 16, color: MitraColors.textPrimary)),
              const SizedBox(height: 8),
              ClipRRect(
                borderRadius: BorderRadius.circular(MitraRadius.pill),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text('13/20 topics', style: TextStyle(fontFamily: 'Mukta', fontSize: 12, color: MitraColors.textMuted)),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(
                      color: MitraColors.gold.withOpacity(0.2),
                      borderRadius: BorderRadius.circular(MitraRadius.pill),
                      border: Border.all(color: MitraColors.gold.withOpacity(0.4)),
                    ),
                    child: const Text('+240 XP', style: TextStyle(fontFamily: 'SpaceMono', fontWeight: FontWeight.w700, fontSize: 11, color: MitraColors.gold)),
                  ),
                ],
              ),
            ],
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 6),
            Text(label, textAlign: TextAlign.center, style: const TextStyle(fontFamily: 'Mukta', fontWeight: FontWeight.w500, fontSize: 10, color: MitraColors.textSecondary)),
          ],
        ),
      ),
    ),
...
class _RankRow extends StatelessWidget {
  ...
  final int xp;
  final bool isMe;
  const _RankRow({required this.rank, required this.name, required this.xp, required this.isMe});

  @override
  Widget build(BuildContext context) => Container(
    padding: const EdgeInsets.all(MitraSpacing.sm),
    decoration: BoxDecoration(
      color: isMe ? MitraColors.saffron.withOpacity(0.1) : MitraColors.bgCard,
      borderRadius: BorderRadius.circular(MitraRadius.sm),
      border: Border.all(color: isMe ? MitraColors.saffron.withOpacity(0.3) : MitraColors.border),
    ),
    child: Row(
      children: [
        SizedBox(width: 32, child: Text(rank, style: const TextStyle(fontSize: 20))),
        Expanded(child: Text(name, style: const TextStyle(fontFamily: 'Mukta', fontWeight: FontWeight.w500, fontSize: 14, color: MitraColors.textPrimary))),
        Text('$xp XP', style: const TextStyle(fontFamily: 'SpaceMono', fontWeight: FontWeight.w700, fontSize: 13, color: MitraColors.gold)),
      ],
    ),
  );
}
